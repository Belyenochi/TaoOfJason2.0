...
2. 所有正整数的编码首位都是0
3. 所有负整数的编码首位都是1
4. 为了通过x的编码获得-x的编码，所有最右边的0和 从左边起的第一个1保持不变，然后将剩余的位取反。等价的捷径就是，对x的所有位取反，然后再加1， **这个方案更容易在硬件中实现** 。 这种表示法使得任何两个用补码表示的有符号数的加法与正数的加法完全相同，比如（1110）+（1101）=1011（丢掉溢出位），刚好等于-5，而减法被等价为x - y = x + （-y）
